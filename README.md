...
- **Honest platform limit, not a bug**: the browser's contact-picker API is restricted by the OS to name/phone/email/address only — prefix, nickname, job title, department, company, websites, birthday, and tags aren't accessible through it on any app, not just this one. Full-fidelity import for those fields is a native-Android-app capability, planned for that later phase.
- **Photo buttons redesigned**: "Choose photo" and "Remove" are now compact, single-line, and consistently sized (previously "Choose photo" wrapped to two lines while "Remove" didn't, so the two buttons visibly didn't match). "Remove" is now styled as a clear warning/danger button, and asks for confirmation before it actually deletes the photo.
- **Arabic now renders in Rubik** instead of falling back to whatever Arabic font the phone happens to ship — loaded from Google Fonts and cached by the service worker after first use so it still works offline afterward. Also fixed a pre-existing gap found while touching the service worker: `pipeline.js` wasn't in the offline precache list at all.
